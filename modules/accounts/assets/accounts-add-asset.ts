...
import { KeysSchema, Keys } from '../accounts-schemas';
...
export class AccountsAddAsset extends BaseAsset {
    ...
    async apply({ asset : { url }, stateStore }: ApplyAssetContext<AccountURL>) {
        ...
        if (keysBuffer){
            gpg_keys = codec.decode<Keys>(KeysSchema, keysBuffer).keys;
        }
        gpg_keys.push({key});
        await stateStore.chain.set("accounts:keys", codec.encode(KeysSchema, { keys:gpg_keys } ));
